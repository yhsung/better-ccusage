...
func TestNewServer_ListsSixTools_Integration(t *testing.T) {
	...
	sort.Strings(names)
	want := []string{"blocks", "codex-daily", "codex-monthly", "daily", "monthly", "session"}
...
func TestServer_CallCodexDaily_Integration(t *testing.T) {
	fake := filepath.Join(t.TempDir(), "fake-codex")
	if err := os.WriteFile(fake, []byte("#!/bin/sh\necho '{\"daily\":[]}'\n"), 0o755); err != nil {
		t.Fatal(err)
	}
	ctx := context.Background()
	srv := NewServer(Opts{CodexBin: fake, DefaultMode: cost.CostAuto, Prices: nil, Version: "test"})
	ct, st := mcp.NewInMemoryTransports()
	client := mcp.NewClient(&mcp.Implementation{Name: "test-client", Version: "test"}, nil)
	srvSess, err := srv.Connect(ctx, st, nil)
	if err != nil {
	...
	defer cliSess.Close()
	res, err := cliSess.CallTool(ctx, &mcp.CallToolParams{Name: "codex-daily", Arguments: map[string]any{}})
	if err != nil {
		t.Fatalf("CallTool: %v", err)
	}
	if res.IsError {
...
func TestServer_CallMonthlySessionBlocks_Integration(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	dir := t.TempDir()
	project := filepath.Join(dir, "projects", "p")
	if err := os.MkdirAll(project, 0o755); err != nil {
...
`
	...
	defer cliSess.Close()
	for _, name := range []string{"monthly", "session", "blocks"} {
		t.Run(name, func(t *testing.T) {
			res, err := cliSess.CallTool(ctx, &mcp.CallToolParams{Name: name, Arguments: map[string]any{"mode": "auto"}})
			if err != nil {
				t.Fatalf("CallTool(%s): %v", name, err)
			}
			if res.IsError {
				t.Fatalf("CallTool(%s): unexpected IsError result: %+v", name, res.Content)
			}
			if len(res.Content) == 0 {
			...
			if !strings.Contains(tc.Text, `"daily"`) {
				t.Errorf("CallTool(%s): expected JSON with daily key, got: %q", name, tc.Text)
			}
		})
	}
}
