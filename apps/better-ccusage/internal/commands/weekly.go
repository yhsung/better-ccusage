...
// WeeklyOpts are the weekly command's options.
type WeeklyOpts struct {
	CommonOpts
}

// Weekly is the public Run entry point for the weekly command.
...
// NewWeeklyCmd returns the cobra command for `better-ccusage weekly` along
// with its bound opts.
//
...
// runs before RunE.
func NewWeeklyCmd(prices *pricing.PriceTable) (*cobra.Command, *WeeklyOpts) {
	opts := &WeeklyOpts{}
	cmd := &cobra.Command{
		Use:   "weekly",
