...
// MonthlyOpts are the monthly command's options.
type MonthlyOpts struct {
	CommonOpts
}

// Monthly is the public Run entry point for the monthly command.
...
// NewMonthlyCmd returns the cobra command for `better-ccusage monthly` along
// with its bound opts.
//
...
// runs before RunE.
func NewMonthlyCmd(prices *pricing.PriceTable) (*cobra.Command, *MonthlyOpts) {
	opts := &MonthlyOpts{}
	cmd := &cobra.Command{
		Use:   "monthly",
